...
const unblockStudent = asyncErrorHandler(async (req, res, next) => {
  ...
  if (!student) {
    throw new CustomError("thisStudentDoesNotExist", 400);
  } else {
  ...
  const response = new Response(
    true,
    null,
    `rollNo${rollNo}UnblockedSuccessfully`,
    "success",
    200,
    null
  );
